...
app.get('/__replit_web_view', (req, res) => {
  res.status(200).send('HugMeNow Web Application is running');
});

// Additional feedback tool endpoints for older versions
...
app.get('/', (req, res, next) => {
  ...
  const isFeedbackCheck = 
    ...
    req.query['_antml'] === 'true' ||
    req.headers['x-antml-feedback'] === 'true';
  
  // Special handling for the web application feedback tool
  if (isFeedbackCheck) {
  ...
  if (acceptsHtml) {
    ...
    // Send the index.html file directly for web application feedback tool
    if (req.headers['x-replit'] || req.headers['x-replit-user-id']) {
      console.log('[REPLIT REQUEST] Sending index.html directly for Replit request');
  ...
  res.status(200).json({ status: 'ok', message: 'HugMeNow Web Application is running' });
});
